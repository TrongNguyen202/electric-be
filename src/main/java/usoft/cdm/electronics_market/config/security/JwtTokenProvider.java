...
public class JwtTokenProvider {
    ...
    public String generateToken(CustomUserDetails userDetails) {
        Date now = new Date();
        int roleId = userDetails.getUsers().getRoleId();
        long expriTimeMillis;
        if (roleService.checkRole(roleId, "CUSTOMER"))
            expriTimeMillis = now.getTime() + (24 * 60 * 60 * 1000);
        else
            expriTimeMillis = now.getTime() + (600 * 60 * 1000);
